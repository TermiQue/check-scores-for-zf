...
启动器只使用“运行状态”和“等待输入”两种过程提示；完成时显示“运行成功”。动态进度条会在同一行刷新百分比和耗时，Docker 正常英文输出会被收纳，仅在失败时附最后 12 行诊断信息。配色分别为启动台 `#f0f5e5`、运行状态 `#AB372F`、运行成功 `#2bae85`、等待输入 `#fbb929`，其他说明使用终端默认文字颜色。
