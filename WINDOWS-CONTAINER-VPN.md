...
| 通知 | 含义 | 恢复方法 |
| --- | --- | --- |
| `VPN连接断开，需要手动重启，累计检测 N 次` | EasyConnect 隧道或 VPN 路由不存在 | 运行 `windows-launcher.cmd`，在 EasyConnect 中重新登录并完成短信验证 |
| `正方系统连接断开，需要手动重启，累计检测 N 次` | VPN 仍连接，但正方页面、接口或登录会话异常 | 运行 `windows-launcher.cmd`，按提示重新验证连接或输入图片验证码 |
| `未知错误` | 尚未分类的程序异常 | 查看 checker 日志，再运行 `windows-launcher.cmd` |

每次失败检测都会增加累计次数；同类故障默认 6 小时内不重复推送，下次通知会显示最新累计次数。故障类型变化会立即通知，连接恢复后会发送一次恢复通知并清零计数。
