...
交互式启动时，每张验证码只提交一次。第一次输入错误后容器会立即把结果返回启动器，不会先在后台累计 5 次失败。启动器随后显示教务系统返回的原因以及当前学号、密码字段；密码默认以掩码显示，需要核对时可以勾选“显示密码”。确认凭据正常后可直接获取新验证码，也可以修改学号和密码后重新验证；只有用户选择“取消启动”时才会停止本次流程，成绩基线和其他本地配置不会被删除。

验证码错误后不会自动连续弹出新图片；只有用户在恢复界面选择“重新输入验证码”时，启动器才会获取一张新图片并重新弹窗。不需要打开资源管理器，也不需要在容器 CLI 中输入。
