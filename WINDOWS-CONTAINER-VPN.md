...
启动器只使用“运行”和“成功”两种任务状态。每项任务开始时在当前行显示 `[运行]`，结束后会在同一行切换为 `[成功]`；需要账号、验证码或 VPN 操作时只显示输入提示，不作为第三种状态。动态进度条会在同一行刷新百分比和耗时，Docker 正常英文输出会被收纳，仅在失败时附最后 12 行诊断信息。配色分别为启动台 `#f0f5e5`、运行 `#AB372F`、成功 `#2bae85`、输入提示 `#fbb929`，其他说明使用终端默认文字颜色。
...
需要登录容器 VPN 时，启动器会先弹出注意事项和完整操作步骤。只有用户确认后才会打开：
