...
状态显示通过 `windows-ui.ps1` 统一实现，任务状态只有“运行”和“成功”。每项任务使用活动状态记录，在开始时以单行 `[运行]` 显示，正常结束后原地重绘为 `[成功]`；人工输入只是提示，不进入状态机。Docker CLI 由 `Invoke-DockerWithProgress` 包装，以同一活动行显示进度条、百分比和耗时。ANSI 真彩色终端使用项目规定的四组十六进制颜色，重定向输出时自动退回无颜色文本。Docker 正常输出被收纳，仅在失败时显示最后 12 行诊断信息。
