...
| 文件 | 职责 |
| --- | --- |
| `compose.easyconnect.yml` | 基础定义。包含 `checker`、`easyconnect` 和仅在 VPN 模式启用的 `push-relay`。 |
...
Python 3.12 容器，入口为 `python -m zfcheck run`。启动器根据实际连通性选择普通 Docker 网络、Docker Desktop Host 网络，或通过 `network_mode: service:easyconnect` 共享 VPN 网络。访问正方不经过应用层 HTTP 代理。在 VPN 模式下，通知交给独立的 `push-relay`，避免公网 ShowDoc 请求被校园 VPN 阻断。
...
| `notify-relay` | 启动内部微信推送中继（由 Compose 管理） |

### `push-relay`

仅在容器 VPN 模式运行的轻量 Python HTTP 服务，不发布任何宿主机端口。启动器取得它的 Docker 内部 IP，并在 EasyConnect 中为该地址添加一条精确的 `/32` 直连路由，因此既不依赖被 VPN 覆盖的容器 DNS，也不会让 ShowDoc 请求进入校园隧道。Push Token 只以 Docker secret 挂载，通知中继失败不会被误判成 VPN 或正方故障。

## 请求路径

### 网络模式选择流程

启动脚本 `windows-launcher.ps1` 按以下决策树选择网络模式：
...
直连模式的通知请求
checker → Docker/Windows → push.showdoc.com.cn

容器 VPN 模式的通知请求
checker → 精确 Docker 直连路由 → push-relay → Docker/Windows → push.showdoc.com.cn
```
...
`windows-launcher.ps1` 是 Windows 编排核心。它通过菜单或 `-Action` 参数统一提供启动、停止、隐私清理、最近日志和实时日志查询。启动操作负责：
...
状态显示通过 `windows-ui.ps1` 统一实现，任务状态只有“运行”和“成功”。每项任务使用活动状态记录，在开始时以单行 `[运行]` 显示，正常结束后原地重绘为 `[成功]`；人工输入只是提示，不进入状态机。普通同步任务由独立计时器持续重绘 `| / - \` 旋转帧，因此主 PowerShell 线程阻塞执行检查时仍有动态反馈；Docker CLI 由 `Invoke-DockerWithProgress` 包装，以同一活动行显示进度条、百分比和耗时。ANSI 真彩色终端使用项目规定的四组十六进制颜色，重定向输出时自动退回无颜色文本。Docker 正常输出被收纳，仅在失败时显示最后 12 行诊断信息。
...
两个服务均配置 `restart: "no"`。Docker Desktop 重启后由用户手动运行 `windows-launcher.cmd`，启动器负责恢复 VPN、登录和后台 checker。
